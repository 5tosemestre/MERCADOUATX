...
@ManagedBean(name = "fileuploadcontroller3")
@RequestScoped
	...
	public String guardarDato(String titulo,String desCort,String descLarg,String precio,String imagen){
		try{
			try {
				...
				String sql = "{ ? = call MERCADOUATX.PAMERCADOUATX.FNINSERTAR_AVISO(?,?,?,?,?,?)} ";
				CallableStatement callableStatement = connection.prepareCall(sql);
				callableStatement.registerOutParameter(1, Types.INTEGER);
				callableStatement.setString(2, titulo);
				callableStatement.setString(3, desCort);
				
				callableStatement.setString(4,precio);
				
				callableStatement.setString(5, descLarg);
				callableStatement.setString(6, imagen);
